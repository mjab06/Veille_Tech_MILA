...
Points clés:
- Respecte robots.txt (lecture au démarrage, journalise les URLs ignorées).
- Sources:
    1) sitemap(s) du site (prioritaire et plus "propre")
    2) pages d’index publications si autorisées
- Filtrage thématique FR/EN.
- Sorties:
    - data/mila_publications.xlsx (colonnes riches)
    - data/ignored_by_robots.csv (artifact listant les sections interdites ignorées)
"""
...
DEFAULT_HEADERS = {
    "User-Agent": "Mozilla/5.0 (compatible; MILA-Publication-Research/1.0; +https://github.com/your-org/your-repo)"
}
TIMEOUT = 30
MAX_RETRIES = 3
...
KEYWORDS = [kw.lower() for kw in [
    ...
    "disease model","patient stratification","therapeutic","trial","biobank","digital twin", "virtual twin",
...
def is_relevant(text: str) -> bool:
    ...
    return hits >= 1  # augmente à 2 pour un filtre plus strict
...
def main():
    ...
    # Export journal robots (toujours créer le CSV, même vide)
